...
const GET_DOC_ENDPOINT = `${API_BASE}/get-document`;
const DELETE_DOCS_ENDPOINT = `${API_BASE}/documents`;
...
export const listDocuments = async (folder = "") => {
  try {
    const url = folder
      ? LIST_DOCS_ENDPOINT + "?folder=" + folder
      : LIST_DOCS_ENDPOINT;
...
export const getDocumentDownloadUrl = async (s3Key) => {
  try {
    const response = await fetch(
      `${GET_DOC_ENDPOINT}?key=${encodeURIComponent(s3Key)}`
    );
...
// Bulk approval request (sends to backend)
export async function submitApprovalRequestBulk(items) {
  try {
    // Step 1: Get presigned URLs
    const presignedRes = await fetch(`${API_BASE}/approvals/presigned`, {
      method: "POST",
    ...
    const { uploadUrls } = await presignedRes.json();

    // Step 2: Upload files directly to S3 using presigned URLs
    const uploadPromises = uploadUrls.map(async (urlData, idx) => {
      ...
      if (!uploadRes.ok) {
        ...
        console.error(`S3 upload failed for ${item.fileName}:`, errorText);
        throw new Error(`Upload failed for ${item.fileName}`);
      }

      return urlData.requestId;
    });

    const requestIds = await Promise.all(uploadPromises);

    // Step 3: Confirm uploads
    const confirmRes = await fetch(`${API_BASE}/approvals/confirm`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    return { created: requestIds.length, requestIds };
  } catch (error) {
...
export async function approveRequest(requestId) {
  try {
    const res = await fetch(
      `${APPROVALS_APPROVE_ENDPOINT}/${encodeURIComponent(requestId)}/approve`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: "{}",
...
export async function rejectRequest(requestId, reason = "") {
  try {
    const res = await fetch(
      `${APPROVALS_APPROVE_ENDPOINT}/${encodeURIComponent(requestId)}/reject`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
