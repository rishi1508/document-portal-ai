...
export const getDocumentDownloadUrl = async (s3Key) => {
 try {
 const response = await fetch(`${GET_DOC_ENDPOINT}?key=${encodeURIComponent(s3Key)}`)
...
export const deleteDocuments = async (keys) => {
 ...
 const response = await fetch(`${DELETE_DOCS_ENDPOINT}?keys=${encodeURIComponent(keysParam)}`, {
 method: 'DELETE',
 })
