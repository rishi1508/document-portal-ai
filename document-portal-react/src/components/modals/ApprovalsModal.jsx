...
const ApprovalsModal = ({ isOpen, onClose }) => {
  ...
  const handleApprove = async (id) => {
    ...
    // Start stage transitions based on typical backend timing
    const stageTimer = setTimeout(() => setActionStage('Parsing document...'), 1500)
    const stageTimer2 = setTimeout(() => setActionStage('Indexing with AI...'), 4000)
    
    try {
      setActionStage('Moving to S3...')
                    ...
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="px-3 py-2 bg-green-600/20 text-green-400 rounded-lg hover:bg-green-600/30 disabled:opacity-50 transition-colors flex items-center gap-2 min-w-[140px]"
                          title="Approve"
                        >
                          {actioning === it.requestId ? (
                            <>
                              <Loader className="w-4 h-4 animate-spin flex-shrink-0" />
                              <span className="text-xs whitespace-nowrap">{actionStage}</span>
                            </>
                          ) : (
